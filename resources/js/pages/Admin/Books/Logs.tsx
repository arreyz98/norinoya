...
import { 
    History, ArrowLeft, Search, Eye, Calendar as CalendarIcon, Clock, Laptop, Smartphone, BookOpen, TrendingUp, Users, RefreshCw, FileSpreadsheet, Trash2, X, AlertTriangle, SearchCode
} from 'lucide-react';
...
interface Props {
    logs: PaginationData<BookViewLogItem>;
    filters?: {
        search?: string;
...
export default function Logs({ logs, filters: rawFilters, stats }: Props) {
    ...
    const [deleteItem, setDeleteItem] = useState<BookViewLogItem | null>(null);
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-5 p-4 sm:p-6">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-2xl font-bold flex items-center gap-2 text-neutral-900 dark:text-white">
                                ...
                                <span>Log Kunjungan &amp; Waktu Buku</span>
                            </h1>
                        </div>
                        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5 ml-10">
                            Pencatatan riwayat waktu real-time saat buku dilihat oleh pengunjung (dengan anti-spam 1 view/menit per IP).
                        </p>
                    </div>
                ...
                <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-3.5 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3 shadow-2xs">
                    ...
                    {/* Right: 2 Date Picker Buttons (Center Dialog Modal) and Reset */}
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {filters.book_id && (
                            <span className="text-xs font-mono font-bold text-neutral-600 dark:text-neutral-300 bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 rounded-md">
                                Filter Book #{filters.book_id}
                            </span>
                        )}

                        {(search || filters.book_id || startDate || endDate) && (
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={handleReset}
                    ...
                    <Table>
                        <TableHeader className="bg-[#112A12] text-white">
                            <TableRow className="border-b border-[#112A12] hover:bg-transparent">
                                ...
                                <TableHead className="text-xs font-bold text-white">Judul Buku / Jilid</TableHead>
                            ...
                            {logs.data.length > 0 ? (
                                                ...
                                                {log.book ? (
                                                    <div className="space-y-0.5">
                                                        <div className="font-bold text-xs text-neutral-900 dark:text-neutral-100 flex items-center gap-1.5">
                                                            ...
                                                            <span className="truncate">{log.book.title}</span>
                                                            <span className="font-mono text-[10px] text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.2 rounded shrink-0">
                                                                Vol {log.book.volume}
                                                            </span>
                                                        </div>
                                                        {log.book.publisher && (
                                                            <div className="text-[10px] font-mono text-neutral-400">
                                                                {log.book.publisher.name}
                                                            </div>
                                                        )}
                                                    </div>
                                                ) : (
                                                    <span className="text-xs text-neutral-400 italic">Buku ID #{log.book_id} (Dihapus)</span>
                                                )}
                                            </TableCell>
                                ...
                                <TableRow>
                                    <TableCell colSpan={7} className="h-32 text-center text-xs text-neutral-500">
                                        Belum ada riwayat log kunjungan yang tercatat sesuai kriteria filter.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                ...
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-neutral-600 dark:text-neutral-400 mt-2">
                            Apakah Anda yakin ingin menghapus data log ID <strong>#{deleteItem?.id}</strong> untuk buku <strong>{deleteItem?.book?.title || 'buku'}</strong>?
                        ...
                        <DialogDescription className="text-xs text-neutral-600 dark:text-neutral-400 mt-2 space-y-2">
                            <p>
                                {startDate || endDate ? (
                                    ...
                                    <>Anda akan menghapus <strong>seluruh data log kunjungan</strong> yang ada di database.</>
                                )}
                            </p>
                            <p className="text-[11px] text-neutral-500">
                                <em>Catatan: Jumlah counter views pada masing-masing buku tidak akan terpengaruh atau berkurang.</em>
                            </p>
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="mt-4 gap-2">
                ...
                <DialogContent className="max-w-lg w-[92vw] sm:w-[500px] p-6 flex flex-col items-center justify-center">
                    <DialogHeader className="w-full text-center sm:text-center pb-2">
                        ...
                        <DialogDescription className="text-xs text-neutral-500">
                            Pilih tanggal awal untuk memfilter data log kunjungan buku.
                        </DialogDescription>
                    </DialogHeader>
            ...
            <Dialog open={isEndDateOpen} onOpenChange={setIsEndDateOpen}>
                <DialogContent className="max-w-lg w-[92vw] sm:w-[500px] p-6 flex flex-col items-center justify-center">
                    <DialogHeader className="w-full text-center sm:text-center pb-2">
                        ...
                        <DialogDescription className="text-xs text-neutral-500">
                            Pilih tanggal akhir untuk memfilter data log kunjungan buku.
                        </DialogDescription>
                    </DialogHeader>
