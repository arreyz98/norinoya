...
export default function NewsEdit({ newsItem, books = [], tiktokEmbeds = [] }: NewsEditProps) {
    ...
    const initialPollOptions = (newsItem.poll_options && newsItem.poll_options.length > 0)
        ? newsItem.poll_options
        : [{ label: '', votes: 85 }, { label: '', votes: 62 }];

    const initialReactions = (newsItem.reactions && newsItem.reactions.length > 0)
        ? newsItem.reactions
        : DEFAULT_REACTIONS;

    const initialRecommendations = (newsItem.recommendations && newsItem.recommendations.length > 0)
        ? newsItem.recommendations.map(r => ({
            number: r.number || 1,
            title: r.title || '',
    ...
    const initialRelevantBooks = (newsItem.relevant_books && Array.isArray(newsItem.relevant_books))
        ? newsItem.relevant_books.map(id => Number(id))
        : [];
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        is_pinned: (!!newsItem.is_pinned) as boolean,

        recommendations: initialRecommendations as Array<{
        ...
        relevant_books: initialRelevantBooks as number[],

        enable_poll: (!!newsItem.poll_question) as boolean,
        poll_question: newsItem.poll_question || '',
        poll_options: initialPollOptions as Array<{ id?: string; label: string; votes: number }>,
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-1 flex-col gap-6 p-4 max-w-4xl mx-auto w-full">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-semibold">Edit Berita & Poling</h1>
                        <p className="text-sm text-muted-foreground">
                            Perbarui berita, galeri gambar (maks 4), hashtag badge, poling, atau jumlah reaksi.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="border border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/40 dark:bg-emerald-950/20 p-5 rounded-xl space-y-4">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <h3 className="font-bold text-sm text-emerald-955 dark:text-emerald-200">
                        ...
                        {data.enable_poll && (
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="space-y-3">
                                    <Label className="text-xs">Pilihan Jawaban & Suara saat Ini</Label>
                    ...
                    <div className="border border-orange-200 dark:border-orange-900/60 bg-orange-50/50 dark:bg-orange-950/20 p-5 rounded-xl space-y-4">
                        <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-bold text-sm text-orange-900 dark:text-orange-200">
                                Angka Reaksi (Boost / Social Proof)
                            </h3>
                        </div>

                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {data.reactions.map((react, idx) => (
                                <div key={react.id || idx} className="bg-card border p-3 rounded-lg space-y-1.5">
