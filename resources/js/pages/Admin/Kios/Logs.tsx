...
import { 
    History, ArrowLeft, Search, Eye, Calendar as CalendarIcon, Clock, Laptop, Smartphone, ShoppingBag, TrendingUp, Users, RefreshCw, FileSpreadsheet, Trash2, X, AlertTriangle, MousePointerClick, SearchCode
} from 'lucide-react';
...
interface Props {
    logs: PaginationData<KiosViewLogItem>;
    filters?: {
        search?: string;
        kios_item_id?: string | number;
        action_type?: string;
        start_date?: string;
        end_date?: string;
    };
...
export default function Logs({ logs, filters: rawFilters, stats }: Props) {
    ...
    const [deleteItem, setDeleteItem] = useState<KiosViewLogItem | null>(null);
    ...
    const handleApplyFilters = (newStart?: string, newEnd?: string, newSearch?: string, newAction?: string) => {
        ...
        const queryAction = newAction !== undefined ? newAction : actionType;

        router.get(
            route('admin.kios.logs'),
            {
                search: querySearch || undefined,
                kios_item_id: filters.kios_item_id || undefined,
                action_type: queryAction !== 'all' ? queryAction : undefined,
    ...
    const renderActionBadge = (action: string) => {
        if (action === 'view') {
            return (
                <span className="inline-flex items-center gap-1 text-[11px] font-mono font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded border border-emerald-200 dark:border-emerald-800">
        ...
        return (
            <span className="inline-flex items-center gap-1 text-[11px] font-mono font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-800 uppercase">
                <MousePointerClick className="w-3 h-3" />
                <span>Klik {platform}</span>
            </span>
        );
    };
            ...
            <div className="flex flex-1 flex-col gap-5 p-4 sm:p-6">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-2xl font-bold flex items-center gap-2 text-neutral-900 dark:text-white">
                                ...
                                <span>Log Kunjungan &amp; Klik Produk Kios</span>
                            </h1>
                        </div>
                        <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5 ml-10">
                            Pencatatan riwayat waktu saat produk dilihat atau link pembelian diklik oleh pembeli.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 shadow-2xs flex items-center justify-between">
                        <div className="space-y-0.5">
                            <span className="text-[11px] font-mono font-bold text-neutral-500 dark:text-neutral-400 uppercase">
                                Total Klik Pembelian
                            </span>
                            <div className="text-2xl font-sans font-black text-amber-600 dark:text-amber-400">
                    ...
                    <form onSubmit={handleSearchSubmit} className="flex items-center gap-2 flex-1 max-w-md">
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                handleApplyFilters(undefined, undefined, undefined, e.target.value);
                            }}
                        ...
                        >
                            ...
                            <option value="click_shopee">Klik Shopee</option>
                            <option value="click_tokopedia">Klik Tokopedia</option>
                            <option value="click_gramedia">Klik Gramedia</option>
                            <option value="click_toco">Klik Toco</option>
                        </select>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {filters.kios_item_id && (
                            <span className="text-xs font-mono font-bold text-neutral-600 dark:text-neutral-300 bg-neutral-100 dark:bg-neutral-800 px-2.5 py-1 rounded-md">
                                Filter Kios #{filters.kios_item_id}
                            </span>
                        )}

                        {(search || filters.kios_item_id || actionType !== 'all' || startDate || endDate) && (
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={handleReset}
                    ...
                    <Table>
                        <TableHeader className="bg-[#112A12] text-white">
                            <TableRow className="border-b border-[#112A12] hover:bg-transparent">
                                ...
                                <TableHead className="text-xs font-bold text-white">Produk / Item</TableHead>
                                <TableHead className="text-xs font-bold text-white">Aktivitas</TableHead>
                                <TableHead className="text-xs font-bold text-white">Waktu &amp; Jam</TableHead>
                            ...
                            {logs.data.length > 0 ? (
                                                ...
                                                {log.kios_item ? (
                                                    <div className="space-y-0.5">
                                                        <div className="font-bold text-xs text-neutral-900 dark:text-neutral-100 flex items-center gap-1.5">
                                                            ...
                                                            <span className="truncate">{log.kios_item.title}</span>
                                                        </div>
                                                        <div className="text-[10px] font-mono text-neutral-400 flex items-center gap-2">
                                                            <span className="bg-neutral-100 dark:bg-neutral-800 px-1 rounded">
                                                                {log.kios_item.is_preloved ? 'Preloved' : 'Partner'}
                                                            </span>
                                                            {log.kios_item.publisher_name && (
                                                                <span>{log.kios_item.publisher_name}</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                ) : (
                                                    <span className="text-xs text-neutral-400 italic">Item Kios #{log.kios_item_id} (Dihapus)</span>
                                                )}
                                            </TableCell>
                                ...
                                <TableRow>
                                    <TableCell colSpan={8} className="h-32 text-center text-xs text-neutral-500">
                                        Belum ada riwayat log aktivitas kios yang tercatat sesuai kriteria filter.
                                    </TableCell>
                                </TableRow>
                            )}
                        </TableBody>
                ...
                <DialogContent className="max-w-lg w-[92vw] sm:w-[500px] p-6 flex flex-col items-center justify-center">
                    <DialogHeader className="w-full text-center sm:text-center pb-2">
                        ...
                        <DialogDescription className="text-xs text-neutral-500">
                            Pilih tanggal awal untuk memfilter data log aktivitas kios.
                        </DialogDescription>
                    </DialogHeader>
            ...
            <Dialog open={isEndDateOpen} onOpenChange={setIsEndDateOpen}>
                <DialogContent className="max-w-lg w-[92vw] sm:w-[500px] p-6 flex flex-col items-center justify-center">
                    <DialogHeader className="w-full text-center sm:text-center pb-2">
                        ...
                        <DialogDescription className="text-xs text-neutral-500">
                            Pilih tanggal akhir untuk memfilter data log aktivitas kios.
                        </DialogDescription>
                    </DialogHeader>
                ...
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2 text-red-600">
                            ...
                            <span>Hapus Riwayat Log Kios</span>
                        </DialogTitle>
                        <DialogDescription className="text-xs text-neutral-600 dark:text-neutral-400 mt-2">
                            Apakah Anda yakin ingin menghapus data log ID <strong>#{deleteItem?.id}</strong> untuk produk <strong>{deleteItem?.kios_item?.title || 'item'}</strong>?
            ...
            <Dialog open={showClearConfirm} onOpenChange={setShowClearConfirm}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2 text-red-600">
                            <AlertTriangle className="w-5 h-5" />
                            <span>Bersihkan Seluruh Log Kios</span>
                        </DialogTitle>
                        <DialogDescription className="text-xs text-neutral-600 dark:text-neutral-400 mt-2 space-y-2">
                            <p>
                                {startDate || endDate ? (
                                    ...
                                    <>Anda akan menghapus <strong>seluruh data log aktivitas kios</strong> yang ada di database.</>
                                )}
                            </p>
                            <p className="text-[11px] text-neutral-500">
                                <em>Catatan: Jumlah counter views dan klik pada masing-masing produk kios tidak akan terpengaruh atau berkurang.</em>
                            </p>
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className="mt-4 gap-2">
